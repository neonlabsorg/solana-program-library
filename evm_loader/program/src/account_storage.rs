        ...
        for (i, account) in (&account_infos).iter().enumerate() {
            ...
            if account.owner == program_id {
                ...
                let code_data = if code_account == Pubkey::new_from_array([0u8; 32]) {
                    ...
                    debug_print!("code_account != Pubkey::new_from_array([0u8; 32])");
                    debug_print!(&("account key:  ".to_owned() + &account.key.to_string()));
                    debug_print!(&("code account: ".to_owned() + &code_account.to_string()));
...
impl<'a> AccountStorage for ProgramAccountStorage<'a> {
    ...
    fn get_contract_seeds(&self) -> Option<(H160, u8)> {
        match self.get_account(&self.contract_id) {
    ...
    fn get_caller_seeds(&self) -> Option<(H160, u8)> {
        match self.get_account(&self.caller_id) {
